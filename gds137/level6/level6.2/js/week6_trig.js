...
function animate()
{
	...
	if(Math.sqrt(Math.pow(player.x-pearl.x,2)+Math.pow(player.x-pearl.x,2))<=300){
		pearl.angle = Math.atan2(pearl.y-player.y,pearl.x-player.x);
		pearl.ax = Math.cos(pearl.angle);
		pearl.ay = Math.sin(pearl.angle);
		pearl.vx += -pearl.ax;
		pearl.vy += -pearl.ay;
		pearl.vx *= fX;
		pearl.vy *= fY;
	...
	if(Math.round(player.x)==Math.round(pearl.x)&&Math.round(player.y)==Math.round(pearl.y)){
		pearl.x = 100000;
	}
